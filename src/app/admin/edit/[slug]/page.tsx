...
import { getArticleBySlug, updateArticle, slugify } from "@/lib/db";
...
export default function AdminEditArticlePage() {
  ...
  useEffect(() => {
    const loadArticle = async () => {
      ...
      try {
        ...
        if (!article) {
          setError("Article not found in the database archives.");
          setIsLoading(false);
          return;
        }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setImageType("upload"); // Automatically toggle upload mode on selecting file
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If upload option is selected and a new file was chosen, upload it
      ...
      await updateArticle(articleId, updatedPayload);
  ...
  if (isLoading) {
    return (
      <div className="space-y-4 animate-pulse">
        <div className="h-6 w-32 bg-editorial-cream-dark/60 rounded-sm" />
        <div className="h-10 w-3/4 bg-editorial-cream-dark/60 rounded-sm" />
        <div className="h-64 w-full bg-editorial-cream-dark/40 rounded-sm" />
      </div>
    );
  }
      ...
      <div className="border-b border-[#e6e2da] pb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-editorial-gray mt-1 font-light">
            Modify the content or settings of the selected long-form article.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {imageType === "upload" ? (
            <div className="border border-dashed border-[#e6e2da] bg-editorial-cream-dark/10 p-8 rounded-sm text-center relative hover:bg-editorial-cream-dark/20 transition-colors">
              ...
              <p className="text-sm font-bold text-editorial-charcoal">
                {selectedFile ? selectedFile.name : "Choose new file to replace current"}
              </p>
            ...
            <div>
              ...
              <span className="text-[10px] font-mono text-editorial-gray mt-1 block">
                Current URL: <code>{coverImageUrl}</code>
              </span>
            </div>
          )}
        </div>
        ...
        <div className="flex justify-end pt-4">
          ...
          >
            ...
            {isSubmitting ? "Saving Changes..." : "Save Article"}
          </button>
        </div>
      </form>
    </div>
